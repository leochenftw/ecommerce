...
class Utils {
    ...
    public static function getCurrentCart($member_id) {
        if (Member::currentUser()) {
            $orders = Order::get()->filter(array('Progress:LessThan' => 2, 'CustomerID' => $member_id, 'isTopupOrder' => false));
        } else {
            $orders = Order::get()->filter(array('Progress:LessThan' => 2, 'Session' => $member_id, 'isTopupOrder' => false));
        }

        return $orders->count() > 0 ? $orders->first() : null;
    }

    public static function getTopupOrder()
    {
        if ($member = Member::currentUser()) {
            $order = Order::get()->filter(array('Progress:LessThan' => 2, 'CustomerID' => $member->ID, 'isTopupOrder' => true))->first();
            if (empty($order)) {
                $order = new Order();
                $order->isTopupOrder = true;
            }
